            ...
            if (token != null && role != null && refreshToken != null)
            {
                ...
                if (role.Equals("Admin"))
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            HttpResponseMessage response = SendDataHelper<IFormFile>.PostFile($"https://localhost:7298/odata/Accounts?choice=1", file, token).Result;
                            if (response.IsSuccessStatusCode) return RedirectToPage("/StudentPages/Index");
