...
namespace PRN231.CPR.Page.Pages.SpecializationPages
{
    public class IndexModel : PageModel
    {
        ...
        public PaginatedList<SpecializationResponse> Specializations { get; set; } = default!;
        public async Task<IActionResult> OnGet(string SearchBy, string sortOrder, string currentFilter, string searchString, int? pageIndex)
        {
            ...
            List<SpecializationResponse> list = new List<SpecializationResponse>();
            ...
            if (token != null && refreshToken != null && role != null)
            {
                ...
                if (role.Equals("Admin"))
                {
                    ...
                    if (!string.IsNullOrEmpty(searchString) && !string.IsNullOrEmpty(SearchBy))
                    {
                        ...
                        {
                            var rs = SendDataHelper<SpecializationResponse>.GetListData($"https://localhost:7298/odata/Specialization/?$filter=contains(Name,\'{searchString}\')", token).Result;
                            if (rs != null)
                                list = rs.ToList();
                            else list = null;
                        }
                        ...
                        {
                            var rs = SendDataHelper<SpecializationResponse>.GetListData($"https://localhost:7298/odata/Specialization/?$filter=contains(Code,\'{searchString}\')", token).Result;
                            if (rs != null)
                                list = rs.ToList();
                            else list = null;
                        }
                    ...
                    {
                        var rs = SendDataHelper<SpecializationResponse>.GetListData($"https://localhost:7298/odata/Specialization", token).Result;
                        if (rs != null)
                            list = rs.ToList();
                        else list = null;
                    }
                    if (list != null && list.Count() != 0)
                    {
                        ...
                        this.Specializations = PaginatedList<SpecializationResponse>.Create(list, pageIndex ?? 1, pageSize);
                    }
                    if (list == null)
                    {
                        var customer = SendDataHelper<SpecializationResponse>.GetRefreshToken(token, refreshToken).Result;
                        if (customer != null)
                        {
                            ...
                            var rs = SendDataHelper<SpecializationResponse>.GetListData($"https://localhost:7298/odata/Specialization", customer.Token).Result;
                            if (rs != null)
                                this.Specializations = PaginatedList<SpecializationResponse>.Create(rs.ToList(), pageIndex ?? 1, pageSize);
